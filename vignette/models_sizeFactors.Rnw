...
\title{diffloop: a computational framework for identifying and analyzing differential DNA loops from sequencing data}
\author{Supporting Information}
\date{\today}
...
d <- DGEList(km_full@counts, group = group)
...
\section*{Discordant loops between replicates}
We observed a number of "discordant" loops, defined as loops that are absent (counts = 0) in one replicate but abundantly present ($> 5$) in the other biological replicate. A log-log comparison of counts within the (A) K562 replicates and (B) MCF-7 replicates is shown below in \textbf{Figure S2}. 

\begin{figure}[h]
<<discordantPlot, echo = FALSE, fig.height = 4, message = FALSE, warning = FALSE,fig.align='center', cache = TRUE>>=
library(cowplot)
library(diffloop)
library(ggplot2)

full <- loopsMake.mango("../data/raw_mango")
full <- updateLDGroups(full, c("HCT116", "K562", "K562", "MCF7", "MCF7"))
...
noCNV_mangoSig <- mangoCorrection(noCNV, FDR = 0.01)

cm <- noCNV_mangoSig@counts
noCNV_mangoSig_concord <- subsetLoops(noCNV_mangoSig, !((cm[,2]>=5 &cm[,3]==0)|(cm[,3]>=5&cm[,2]==0)|
                                                      (cm[,4]>=5 &cm[,5]==0)|(cm[,5]>=5&cm[,4]==0)))
comp <- noCNV_mangoSig_concord[,c(2:5)]
d <- subsetLoops(noCNV_mangoSig, ((cm[,2]>=5 &cm[,3]==0)|(cm[,3]>=5&cm[,2]==0)|
                                          (cm[,4]>=5 &cm[,5]==0)|(cm[,5]>=5&cm[,4]==0)))
counts <- noCNV_mangoSig@counts
a <- dim(d)
b <- sum(rowSums(d@counts[,c(2,3,4,5)] >= 2) >= 2)

##################
#FigureX
##################
dfb <- data.frame(cbind(x = counts[,2], y = counts[,3]))
bboo <- (dfb[,1] >= 5 & dfb[,2] == 0)|(dfb[,1] == 0 & dfb[,2] >= 5)
fX<- qplot(log2(dfb[,1] + .125), log2(dfb[,2] + .125))+labs(title = "K562 Replicates",
    x = "log2(counts + 1/8) K562 Rep 1", y = "log2(counts + 1/8) K562 Rep 2")+
     geom_point( aes(colour = bboo)) +theme(legend.position="none")+
    scale_color_manual("Status", values = c("black", "firebrick"))

##################
#FigureY
##################
dfc <- data.frame(cbind(x = counts[,4], y = counts[,5]))
cboo <- (dfc[,1] >= 5 & dfc[,2] == 0)|(dfc[,1] == 0 & dfc[,2] >= 5)
fY <- qplot(log2(dfc[,1] + .125), log2(dfc[,2] + .125))+labs(title = "MCF7 Replicates",
    x = "log2(counts + 1/8) MCF7 Rep 1", y = "log2(counts + 1/8) MCF7 Rep 2")+
     geom_point( aes(colour = cboo))+theme(legend.position="none")+
    scale_color_manual("Status", values = c("black", "firebrick"))

plot_grid(fX, fY, ncol = 2, labels = c('A','B'), label_size = 12)
@
\centering
\caption{\textbf{Figure S2.}  Concordance between the (A) K562 and (B) MCF-7 replicates. The 337 loops marked in red each have one replicate with 0 PET counts and the other replicate with 5 or more counts.}
\end{figure}

As these discordant loops were present after our initial filtering steps of preprocessing by removing CNV regions and Mango q-value thresholding, they were a feature of our dataset when performing the principal component analyses. However, 166 of the 337 were filtered (using  filterLoops()) when restricting loops such that 2 samples had at least 2 PETs supporting the loop. 

\section*{Loop Width Stratification}
Though the size factor correction as implemented in diffloop adjusts for variable read depth between samples (Figure 2), we considered whether or not the size factor should vary with the loop width spanning the two anchors. For example, the plot below of size factors calculated within loop width strata shows a potential weak dependence on loop width (\textbf{Figure S3}). 
...
ggplot(mdf, aes(x = Max, y = value, group = Replicate, color = Replicate)) + 
  geom_line() + geom_point() +
    theme_bw() + theme_Publication()+labs(title = "Summary of size factors across variable loop widths", 
    x = "log10 loop width", y = "Size Factor",  fill = NULL) 
@
\centering
\caption{\textbf{Figure S3.} Variable size factor across different loop widths (distance spanned between the midpoint of the anchors).}
\end{figure}

To assess the effect of variable loop widths, we binned loops based on their distance spanned using the groups above (increments of 0.3 on the $\log_{10}$ scale) and assessed differential looping within each stratum individually. \textbf{Figure S4} shows a per-loop comparison of the $-\log_{10}$ ratio of the q-values using the standard model compared to the stratified model. Loops annotated in red and blue are shown with summary statistics in \textbf{Table S1} and \textbf{Table S2}.
...
Outlier <- as.numeric(-log10(mdf$FDR/mdf$FDR_chunk) > 50)
Outlier <- as.numeric(-log10(mdf$FDR/mdf$FDR_chunk) < -50) + Outlier*2
...
@
\centering
\caption{\textbf{Figure S4.} Comparison of per-loop statistical significance with using the standard unstratified model vs. a stratified model based on loop width. Loops marked in red are more significant ($> 50$ orders of magnitude) when using the unstratified model between the tests whereas the blue loops are more significant ($>50$ orders of magnitude) when using the stratified model. The Y axis shows the $-\log_{10}$ ratio of the q-values. \newline}
\end{figure} \newpage \newpage 
...
@
\centering
\caption{\textbf{Table S1.} Summary statistics of the loops marked in red in \textbf{Figure S4.} These loops were highlighted due to large differences in the summary statistic measures such that the unstratified model yielded a less significant test statistic. Individual replicate PET counts, the distance the loop spans, as well as the q-values for each test are displayed. \newline } 
\end{figure}
...
@
\centering
\caption{\textbf{Table S2.} Summary statistics of the loops marked in blue in \textbf{Figure S4.} These loops were highlighted due to large differences in the summary statistic measures such that the unstratified model yielded a more significant test statistic. Individual replicate PET counts, the distance the loop spans, as well as the q-values for each test are displayed.}
\end{figure}  \clearpage

We note that all the highlighted loops are deeemed highly statistically significant in both stratified and unstratified analyses. To explore whether the two models identify different sets of differential loops we plotted the $-\log_{10}$ q-values against each other after applying a ceiling of 4 (q = 0.0001) (\textbf{Figure S5}). Of note, few loops tended to deviate significantly in this range indicating that either approach produces a very similar set of differential loops. Thus, in the present implementation of diffloop, a single unstratified model is applied and the loop width is not considered. As more topology libraries become available, novel methods of association, including those that account for loop width, may be incorporated in the package as the evidence demands.  
...
@
\centering
\caption{\textbf{Figure S5.} A scatter plot comparing the $-\log_{10}$ FDR q-values between 0 and 4 of the fixed and binned association tests. For either test, $-\log_{10}$ FDR q-values greater than 4 were shrunk to 4 for visualization. Loops marked in red were differential (q-value $<0.01$) in the binned association test.} 
\end{figure}  \clearpage


\section*{Comparison with Voom}
As an alternative model to Negative Binomial regression for RNA-Seq data, the limma-voom method approximates the mean-variance relationship of the log-counts and applies a moderated t-test. We implemented a similar approach for finding differential loops analogous to differential transcripts in this model. \textbf{Figure S6} shows a per-loop comparison of the Negative Binomial regression (edgeR) and the voom q-values as a function of loop width. We also highlight the largest deviants in \textbf{Table S3}. 
...
Outlier <- as.numeric(-log10(mdf$edgeR/mdf$Voom) > 80)
Outlier <- as.factor(Outlier)
...
@
\centering
\caption{\textbf{Figure S6.} Comparison of per-loop statistical significance using models from edgeR and Voom. Loops highlighted in orange were assigned a much more statistically significant q-value in edgeR ($> 50$ orders of magnitude) than voom. These loops are shown in \textbf{Table S3}.}
\end{figure}
...
@
\centering
\caption{\textbf{Figure S7.} Comparison of per-loop statistical significance using models from edgeR and voom. Loops highlighted in orange were indicated as the edgeR model provided more statistically significant FDR value. These loops are shown in \textbf{Table S4}.}
\end{figure}
...
@
\centering
\caption{\textbf{Table S4.} Summary statistics of the loops marked in orange in \textbf{Figure S7.} These loops were highlighted due to differences in the summary statistic measures such that they were differential (FDR $< 0.01$) in the edgeR model but not differential (FDR $> 0.1$) in the voom model.}
\end{figure} \clearpage
